...
void AQ_2::operation_handler(struct input_struct* input, struct output_struct* output, struct analytical_driver_config* config){
  ...
  json request;
  request["OPERATION_ID"] = 18;
  request["PR_MAX_ITERS"] = config->query_2_configs.pr_max_iterations;
  request["PR_DAMPING_FACTOR"] = config->query_2_configs.pr_damping_factor;
